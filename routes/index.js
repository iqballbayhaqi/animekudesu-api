...
router.get("/ongoing-anime", function (req, res, next) {
  axios.get(`${process.env.SCRAPE_URL}/daftar-anime-2/?status=Currently+Airing`).then((response) => {
    const html = response.data;
    const $ = cheerio.load(html);
    const page_section = $("#main > div.relat > div > span:nth-child(1)")
      .text()
...
router.get("/search-anime", function (req, res, next) {
  ...
  axios
    .get(
      `${process.env.SCRAPE_URL}/daftar-anime-2/page/${req.query.page || "1"}/?title=${req.query.search}&status=&type=&order=title`
    )
    .then((response) => {
      const html = response.data;
      const $ = cheerio.load(html);
